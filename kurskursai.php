...
<html>
 ...
 <?php
		include("include/functions.php"); //įterpiamas meniu pagal vartotojo rolę
		if (isset($_POST["ok"])){
...
<?php
	...
	if(isset($_POST['edit'])){
		...
		if (!$result || (mysqli_num_rows($result) != 1))   // jei >1 tai DB vardas kartojasi, netikrinu, imu pirma
			...
			$sql = "INSERT INTO " .TBL_CONN ." (kursanto_ak, kurso_id) VALUES ('$asmens', '$kursid')";
			if (!mysqli_query($db, $sql))  die ("Klaida irasant:" .mysqli_error($db));
			else{
				...
				if (!mysqli_query($db, $sql))  die ("Klaida pridedant vietu sk:" .mysqli_error($db));
					...
					$_SESSION['added']="<p style='color:#4682B4;'>* Kursas pridetas";
					header("Location: kurskursai.php");
					exit;
				}
			}
		}
...
<?php
	...
	{	
		...
		if($block == 0 && $space > 0 && !checkdbCourse($_SESSION['userid'], $id) && $datetill > date("Y-m-d")){
			...
			echo $type."</td><td>";
			echo $lname." ".$last."</td>";
			echo "<form method=\"post\"> <td> <input type=\"submit\" name=\"edit[]\", value=\"Pasirinkti\"> <input type=\"hidden\" name=\"edit[]\" value=\"".$id."\" > </td></form></tr>";
		}

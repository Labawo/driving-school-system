...
if (!isset($_SESSION['prev']) || ($_SESSION['prev'] != "registerA"))
{ header("Location: logout.php");exit;}
  ...
  $_SESSION['prev'] = "procregister";
		...
		{ // vardas  geras,  nuskaityti vartotoja is DB
		 ...
		 else $ulevel=$user_roles[ADMIN_LEVEL]; 
    ...
    if(!preg_match("/^([a-zA-Z])*$/", $vardas)) {
		...
		if($_SESSION['vardas_error'] != "" || $_SESSION['pavarde_error'] != "") {
			$conn->close();
			header("Location:registerA.php");exit;
		}
    //---------------------------------------------------------------------------------------------------------------------------------------
     ...
     $ak=substr(hash('sha256',$ak),5,12);
		 ...
		 if (mysqli_query($db, $sql)) 
		      {
            $sql = "INSERT INTO " . TBL_ADMIN. " (vardas, pavarde, id)
          VALUES ('$vardas', '$pavarde', '$userid')";
